# Manual smoke: calls refactor_plan_with_opus via stdio JSON-RPC.
...
# The tool only PROPOSES a refactoring plan; it never applies it. Cursor/user must review,
...
if ($Help) {
    Write-Output @"
smoke-refactor-plan.ps1 - manual smoke for refactor_plan_with_opus (stdio JSON-RPC)

USAGE:
  powershell -ExecutionPolicy Bypass -File scripts/smoke-refactor-plan.ps1
  powershell -ExecutionPolicy Bypass -File scripts/smoke-refactor-plan.ps1 -Context "<context>"
  powershell -ExecutionPolicy Bypass -File scripts/smoke-refactor-plan.ps1 -Help
...
EXPECTED STATUSES:
  positive  -> status=OK, structured refactoring plan with steps
...
NOTES:
  ...
  - The tool only PROPOSES a plan; it never applies it. Review and implement manually.
...
$argsObj = [ordered]@{
    task              = "Plan a readability refactor for this method while preserving behavior"
    language          = "java"
...
$messages = @(
  ...
  ('{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"refactor_plan_with_opus","arguments":' + $argsJson + '}}')
)
