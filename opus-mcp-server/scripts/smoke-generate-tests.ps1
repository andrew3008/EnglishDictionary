# Manual smoke: calls generate_tests_with_opus via stdio JSON-RPC.
...
# The tool only PROPOSES tests; it never runs them. Cursor/user must review, apply, and run manually.
#
...
if ($Help) {
    Write-Output @"
smoke-generate-tests.ps1 - manual smoke for generate_tests_with_opus (stdio JSON-RPC)

USAGE:
  powershell -ExecutionPolicy Bypass -File scripts/smoke-generate-tests.ps1
  powershell -ExecutionPolicy Bypass -File scripts/smoke-generate-tests.ps1 -Context "<context>"
  powershell -ExecutionPolicy Bypass -File scripts/smoke-generate-tests.ps1 -Help
...
EXPECTED STATUSES:
  positive  -> status=OK, structured test proposal with test cases
  .env ref  -> status=REFUSED_UNSAFE (external model call blocked)

NOTES:
  - Only synthetic, non-proprietary code is used; no repository context is sent.
  - The tool only PROPOSES tests; it never runs them. Review and run manually.
...
$argsObj = [ordered]@{
    task          = "Generate unit tests for this method, focusing on overflow edge cases"
    language      = "java"
...
$messages = @(
  ...
  ('{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"generate_tests_with_opus","arguments":' + $argsJson + '}}')
)
