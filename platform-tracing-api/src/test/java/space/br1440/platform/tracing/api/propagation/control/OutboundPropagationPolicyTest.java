...
@DisplayName("OutboundPropagationPolicy: trusted gating + per-header decision")
class OutboundPropagationPolicyTest {
    ...
    private static OutboundPropagationPolicy policy(boolean enabled, boolean force, boolean qa, boolean reqId) {
        return new OutboundPropagationPolicy(
                enabled,
                new TrustedDestinationMatcher(List.of("*.trusted.com")),
                force, qa, reqId);
    }
