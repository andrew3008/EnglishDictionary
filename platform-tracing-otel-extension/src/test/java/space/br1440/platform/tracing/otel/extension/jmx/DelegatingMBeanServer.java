...
/**
 * Тестовый делегат MBeanServer: все вызовы проксируются к реальному серверу.
 * Подклассы переопределяют нужные методы для spy/fault-injection без Mockito.
 */
class DelegatingMBeanServer implements MBeanServer {

    protected final MBeanServer delegate;

    DelegatingMBeanServer(MBeanServer delegate) {
