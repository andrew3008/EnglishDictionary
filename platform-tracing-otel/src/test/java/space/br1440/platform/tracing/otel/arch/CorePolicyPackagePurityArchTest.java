...
/**
 * PR-1 forward-looking guardrail for pure policy packages in otel.
 * <p>
 * {@code otel.sampling} is active (PR-6B). {@code otel.validation} is active (PR-9C).
 * Future {@code otel.{scrubbing,enrichment}} packages are guarded the same way. Facade/span packages remain OTel-coupled (MIGRATION_RISK).
...
class CorePolicyPackagePurityArchTest {
    ...
    // (ApiModuleTaxonomyArchTest), т.к. @AnalyzeClasses этого теста импортирует только otel.
    @ArchTest
    ...
    static final ArchRule rootPackageMustBeEmpty =
            noClasses()
                    .should().resideInAPackage("space.br1440.platform.tracing.otel")
                    .because("root package space.br1440.platform.tracing.otel должен быть пустым после PR-3");

    @ArchTest
    static final ArchRule noImplPackages =
            noClasses()
                    .should().resideInAnyPackage("..impl..")
                    .because("пакет otel.impl удалён");
}
