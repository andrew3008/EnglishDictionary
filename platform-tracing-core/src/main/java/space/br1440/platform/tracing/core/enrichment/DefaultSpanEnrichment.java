...
final class DefaultSpanEnrichment implements SpanEnrichment {

    private static final Logger log = LoggerFactory.getLogger(DefaultSpanEnrichment.class);
    ...
    DefaultSpanEnrichment(@Nonnull Span span, @Nonnull AttributePolicy policy, @Nonnull SpanCategory category) {
        this.span = span;
        this.policy = policy;
    ...
    @Override
    @Nonnull
    public <V> SpanEnrichment attribute(@Nonnull AttributeKey<V> key, @Nonnull V value) {
    ...
    @Override
    @Nonnull
    public SpanEnrichment result(@Nonnull SpanResult result) {
