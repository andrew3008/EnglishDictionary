package space.br1440.platform.tracing.core.naming;
...
/**
 * Строит low-cardinality имя span'а по категории и нормализованным атрибутам (форма из
 * {@code opentelemetry-java-instrumentation} SpanNameExtractor'ов):
 * <ul>
 ...
 * </ul>
 * Имена строятся ДО {@code startSpan()} из ЕДИНОГО snapshot нормализованных атрибутов. Если
 * необходимых атрибутов нет — используется явно заданное имя, иначе низко-кардинальный fallback
 * по категории.
 */
public final class PlatformSpanNameBuilder {

    private PlatformSpanNameBuilder() {
        // utility-класс
    }
