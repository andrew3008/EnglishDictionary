...
| `ExtensionDefaults` | `.../ExtensionDefaults.java` | Platform default values for agent properties | public | `PlatformHeaders`, `BuiltInSensitiveDataRules` | Factories, `ExtensionConfig`, tests | `ExtensionConfigTest`, `ExtensionEnumsTest`, `SharedDefaultsAlignmentTest` (partial) | **HIGH** — default drift breaks startup |
