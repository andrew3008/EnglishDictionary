...
2. **Единый источник истины — `platform-tracing-api`.** Все outbound-контракты (`PlatformPropagationDecision`, `PlatformTraceContextKeys`, `PlatformTraceControl`, `TrustedDestinationMatcher`, `OutboundPropagationPolicy`, `PlatformOutboundInjector`, `RequestIdSupport`) живут в `platform-tracing-api` — видны и app-classloader (Spring-бины), и agent-classloader (extension). `PlatformTraceControlPropagator.inject()` делегирует в `PlatformOutboundInjector` (нет дрейфа логики).
