...
- OpenTelemetry Java Agent + `platform-tracing-otel-extension` (Agent ClassLoader);
...
1. **Boundary blur:** `platform-tracing-otel-extension` совмещает OTel SPI wiring, policy logic, scrubbing, validation, JMX control plane и resource enrichment — пять concerns в одном Agent CL модуле.
...
| Pure policy core | `platform-tracing-core` | Узкий pure-Java policy engine: sampling state, scrubbing rules, validation rules, immutable snapshots |
| OTel adapter | `platform-tracing-otel-extension` | Thin OTel SPI adapters → delegate to core |
...
| `platform-tracing-core` | Both | **Да** | Pure-Java policy engine |
| `platform-tracing-otel-extension` | Agent | **Да** | OTel SPI thin adapters |
...
- **Classloader note:** `platform-tracing-core` может загружаться в Agent CL и App CL как один и тот же bytecode, но **разные Class objects**. Core не должен полагаться на cross-CL object identity, mutable static shared state или прямую передачу объектов через границу CL.
...
1. Extract narrow `platform-tracing-core` (pure Java policy engine).
2. Reduce `platform-tracing-otel-extension` to thin OTel SPI adapters.
