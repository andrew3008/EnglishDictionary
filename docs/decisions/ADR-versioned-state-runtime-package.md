...
- **Agent visibility:** `platform-tracing-otel-extension` already depends on `platform-tracing-otel`; `agentExtensionJar` embeds core classes (same pattern as `core.propagation.control`).
