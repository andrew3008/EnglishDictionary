...
19 июля 2026 года архитекторы приняли решение `APPROVED AS PROPOSED`:

- exact signatures из раздела 2 утверждены без изменений;
- ownership, null/no-op/error semantics и intentional ABI delta утверждены;
- CP-C2 закрыт, Slice C2 разрешён к реализации;
- любые дополнительные public-типы или изменения signatures требуют нового архитектурного решения.
