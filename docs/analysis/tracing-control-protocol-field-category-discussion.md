...
├── type: TracingControlProtocolFieldType   // wire type (INTEGER, DOUBLE, …)
...
| **type** | `TracingControlProtocolFieldType` | Формат wire value (coercion, normalization) |
...
- wire type coercion (`TracingControlProtocolFieldType`);
...
- Production Javadoc в schema package сейчас минимален (см. `TracingControlProtocolFieldType.ratioBounded()` — есть Javadoc на English).
...
- [ ] Упомянуто отличие category от `TracingControlProtocolFieldType`
