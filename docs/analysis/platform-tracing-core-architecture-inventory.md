...
|---------|------------|-------------|
| Публичный фасад v3 | `DefaultTraceOperations`, `NoopTraceOperations` | Реализует `TraceOperations`: `traceContext()` + `manual()` |
| Ручная трассировка (v3) | `core.manual`, `core.impl` | Builder'ы → `SpanSpec` → `TracingImplementation.startSpan()` |
| Legacy OTel-direct builders | `core.span` | Прямой `Tracer.spanBuilder()`; API `api.span.builder.*` — **не подключён к `TraceOperations`** |
...
├── (root)              — фасад TraceOperations, propagation
...
classDiagram
    ...
    DefaultTraceOperations --> TracingImplementation
    DefaultTraceOperations --> DefaultManualTracing
```

| Класс | Видимость | Назначение |
|-------|-----------|------------|
| `DefaultTraceOperations` | `public` | Основная реализация `TraceOperations`. Делегирует в `TracingImplementation` + создаёт `DefaultManualTracing`. Конструкторы: от `TracingImplementation`, от `OpenTelemetry` (+ `AttributePolicy`, `ExceptionRecorder`). Извлекает `AttributePolicy` из `DefaultTracingImplementation` через unwrap делегатов. |
| `NoopTraceOperations` | `public` | Singleton `INSTANCE` и factory `backedBy()`. Manual tracing работает, но span'ы — no-op. |
...
**Смежный код:** `TracingCoreAutoConfiguration` создаёт beans `TraceOperations`, `TracingImplementation`, `PlatformContextPropagation`. `MeteredTracingImplementation` (autoconfigure) оборачивает default impl метриками.
...
**Статус legacy stack:** `TraceOperations` (v3) **не экспонирует** `api.span.builder.*`. Legacy builders остаются в core как dead code для приложений, но используются внутри `SpanEnricher` tests и потенциально прямым инстанцированием (не через public API).
...
```
Direct construction (не через TraceOperations)
...
|-----------|--------------|
| `api.TraceOperations` | Реализуется `DefaultTraceOperations` |
...
|-------|-------------------|
| `TracingCoreAutoConfiguration` | `DefaultTraceOperations`, `DefaultTracingImplementation`, `NoOpTracingImplementation`, propagation |
...
| `TracingHealthIndicator` | `NoopTraceOperations` |
...
| Entry | `traceOperations.manual()` | Нет public entry |
...
| Класс | Тип |
|-------|-----|
| `DefaultTraceOperations` | Facade impl |
| `NoopTraceOperations` | Facade noop |
