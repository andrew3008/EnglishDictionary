# Выжимка из *Observability Engineering* для TraceOperations
...
> **Цель:** не пересказ книги, а извлечь идеи, которые **подтверждают, уточняют или ставят под вопрос** текущее решение TraceOperations v3.
...
Для TraceOperations наиболее ценны главы:

| Глава | Тема | Прямая связь с TraceOperations |
|-------|------|--------------------------------|
...
**Практический вывод:** TraceOperations не должен «прятать» OTel — он должен **управлять** тем, как developers добавляют контекст поверх auto-instrumentation Agent'а.
...
**Формула книги = формула TraceOperations v3:**
```
Agent auto-instrumentation (default) + TraceOperations manual() (exception path)
```
...
**Прямое попадание в ADR TraceOperations:**
...
**TraceOperations mapping:** `CategoryContracts`, STRICT mode, typed builders, `SpanSpec` governance.
...
**Разделение ответственности TraceOperations:**
...
## 3. Mapping: книга → TraceOperations v3
...
App (Spring)
  ...
  → Tempo/Jaeger
       ↑
  TraceOperations manual() — governance facade, не replacement OTel SDK
```

| Тезис книги | Решение TraceOperations | Статус |
|-------------|-------------------------|--------|
...
1. **Platform library, не tracing backend** — книга отдельно выделяет instrumentation libraries vs pipeline vs datastore. traceOperations = **library + policy**, не Tempo/Jaeger replacement.

2. **Head sampling не «чинится» в SDK** — SRE должны владеть tail policies; head — cost guard. Это **контракт OTel**, не баг TraceOperations.
...
## 4. Рекомендации для TraceOperations (actionable)
...
## 6. Связь с существующими артефактами TraceOperations
