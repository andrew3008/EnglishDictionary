...
| `PlatformSamplingReasons` | utility class | `attributes` | Sampling reason constants | `FORCE_HEADER`, `QA_TRACE`, etc. | `InboundTraceControl` | extension sampler | OK |
| `RequestTraceContextSnapshot` | record | `context` | Error-handling snapshot | `correlationId`, `traceId`, `spanId` | distinct from `ActiveTraceContextView` | autoconfigure filters | **Similar concept** to `ActiveTraceContextView` |
...
| `InboundTraceControl` | propagation model | Inbound trace control | Clear record | — | web stack | Low |
...
- Links: `linkedTo` + `fromTraceparent` на обоих уровнях.
...
6. **`ActiveTraceContextView` vs `RequestTraceContextSnapshot`** — два context snapshot типа.
...
3. Evaluate SpecifiedSpan, DatabaseTracing mixin, ActiveTraceContextView vs RequestTraceContextSnapshot.
