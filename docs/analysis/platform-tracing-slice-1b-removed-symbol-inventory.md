# TraceOperations Slice 1B Removed-Symbol Inventory
...
- ALLOW_EXISTING_V1_UNTIL_CUTOVER: **26** (declarations on `TraceOperations.java` itself, kept until atomic cutover)
- REVIEW_REQUIRED: **0**
...
- High-risk modules: `platform-tracing-api` (facade + Facade builders + SpanRelation), `platform-tracing-spring-boot-autoconfigure` (MeteredPlatformTracing, TracingActuatorEndpoint, TracedAspect, KafkaBatchLinksAspect), `platform-tracing-core` (DefaultTraceOperations, AbstractPlatformSpanBuilder, v1 characterization/baseline tests)
...
4. Declarations of old v1 API on `TraceOperations.java` itself were catalogued once by line number rather than duplicated per caller.
...
|---|---|---|---|---|
| `currentTraceId()` | `platform-tracing-api/src/main/java/.../TraceOperations.java` | L35 (declaration) | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove declaration in 1B cutover |
...
| `currentSpanId()` | `platform-tracing-api/src/main/java/.../TraceOperations.java` | L41 (declaration) | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove declaration in 1B cutover |
...
| Symbol | File | Line/Context | Classification | Slice 1B action |
|---|---|---|---|---|
| `startSpan(name, category)` (abstract) | `TraceOperations.java` | L60 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; primary replacement `manual().operation(name).start/run/call` |
| `startSpan(name, category, relation)` (default) | `TraceOperations.java` | L73-76 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove (root cause of R01 default-dispatch) |
| `startRootSpan(...)` (default) | `TraceOperations.java` | L83-84 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().operation(name).root().start/run/call` |
| `startChildSpan(...)` (default) | `TraceOperations.java` | L91-92 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().operation(name).child().start/run/call` |
| `startDetachedSpan(...)` (default) | `TraceOperations.java` | L99-100 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().operation(name).detached().start/run/call` |
| `startSpanWithLinks(...)` (default) | `TraceOperations.java` | L111-114 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `.root().linkedTo(...)` (ROOT+links only; CHILD+links forbidden in v1) |
| `addLink(...)` (default no-op) | `TraceOperations.java` | L120 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; no post-start replacement — pre-start `linkedTo(...)` only |
| `inSpan(...)` × 4 overloads (default) | `TraceOperations.java` | L283, L308, L327, L345 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `run()/call()/callChecked()` |
| `startSpan(name, category)` | `DefaultTraceOperations.java` (concrete impl) | (implements abstract method) | REPLACE (impl migrates to `TracingImplementation.startSpan(SpanSpec)` in Slice 2, not 1B) | Out of 1B scope; noted for Slice 2 planning |
| `addLink(...)` | `DefaultTraceOperations.java` | L182, L225 (calls into OTel `SpanBuilder.addLink`) | REVIEW_REQUIRED → resolved FALSE_POSITIVE for symbol-removal purposes | This is `SpanBuilder.addLink` (OTel SDK), invoked *inside* the v1 implementation of the platform `addLink`/link-handling logic — implementation detail, not the public `TraceOperations.addLink` call site. Will be reworked when `DefaultTracingImplementation` is built in Slice 2. |
| `addLink(...)` | `AbstractPlatformSpanBuilder.java` (via `KafkaBatchLinksAspect.java` L64) | L64 | FALSE_POSITIVE for public symbol removal | OTel `SpanBuilder.addLink`, not `TraceOperations.addLink` |
...
| `startSpan(name, category)` | `TracingCoreAutoConfiguration.java` | L149 (`.startSpan()` on OTel `SpanBuilder`) | FALSE_POSITIVE | OTel SDK call, not `TraceOperations` |
...
| `startSpan(...)` | `platform-tracing-bench/.../TypedBuilderBenchmark.java`, `CompositePipelineBenchmark.java` (×4), `TracedAspectBenchmark.java`, `StartSpanBenchmark.java` | multiple | REPLACE | JMH benchmarks call `TraceOperations.startSpan`; migrate to v3 API or isolate as v1 benchmark baseline until Slice 1B lands |
| `startSpan(...)` | `platform-tracing-bench/.../ValidatingSpanProcessorBenchmark.java`, `SpanLimitsBenchmark.java`, `QueueOfferBenchmark.java` | multiple | FALSE_POSITIVE | OTel SDK `Tracer.spanBuilder(...).startSpan()`, not `TraceOperations` |
...
| `startSpan(...)` | `platform-tracing-spring-boot-autoconfigure/src/test/.../aspect/TracedAspectTest.java` | L96, L133 | ISOLATE_V1_CHARACTERIZATION_TEST | Tests `TracedAspect` against v1 `TraceOperations.startSpan`; migrates together with production `TracedAspect` in Slice 7 |
...
| `startSpan(...)` (various) | `platform-tracing-otel-extension/**` (30+ test files) | many | FALSE_POSITIVE | All confirmed OTel SDK `Tracer.spanBuilder(...).startSpan()`; this module tests span processors directly against the OTel SDK, not `TraceOperations` |
| `startSpan(...)` (Javadoc example) | `platform-tracing-test/.../junit/OtelSdkTest.java` | L33 | FALSE_POSITIVE | Javadoc usage example of OTel SDK, not `TraceOperations` |
...
|---|---|---|---|---|
| `startHttpServer` | `TraceOperations.java` | L128-129 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().operation(name)` or `manual().transport().http()` in Slice 3A |
| `startHttpClient` | `TraceOperations.java` | L136-137 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; same as above |
| `startDb` | `TraceOperations.java` | L144-145 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().transport().database()` (Slice 3B) |
| `startRpcServer` | `TraceOperations.java` | L152-153 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().transport().rpc()` (Slice 3C) |
| `startRpcClient` | `TraceOperations.java` | L160-161 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; same as above |
| `startInternal` | `TraceOperations.java` | L171-172 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; replacement `manual().operation(name)` |

No call sites of these typed shortcuts were found outside `TraceOperations.java` itself and its own default-method bodies (they delegate to `startSpan(name, category)` internally).

### Builder factory methods

| Symbol | File | Line | Classification | Slice 1B action |
|---|---|---|---|---|
| `internalSpan()` | `TraceOperations.java` | L183 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove; returns `Facade*` builder (bypasses validation) |
| `httpServerSpan()` | `TraceOperations.java` | L198 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `httpClientSpan()` | `TraceOperations.java` | L206 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `databaseSpan()` | `TraceOperations.java` | L214 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `rpcServerSpan()` | `TraceOperations.java` | L222 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `rpcClientSpan()` | `TraceOperations.java` | L230 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `kafkaProducerSpan()` | `TraceOperations.java` | L238 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
| `kafkaConsumerSpan()` | `TraceOperations.java` | L246 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Remove |
...
| `SpanRelation` usage | `TraceOperations.java` (L84, L92, L100) | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Removed together with default methods |
| `SpanRelation` usage | `DefaultTraceOperations.java` | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Migrated to `Topology`/`SpanOptions` when `DefaultTracingImplementation` lands (Slice 2); v1 impl kept intact through 1B per plan (only facade cuts over in 1B; core impl detail migration is Slice 2 per plan text, but the `TraceOperations`-facing surface must compile against the new facade — see High-risk findings) |
| `SpanRelation` usage | `platform-tracing-core/src/test/.../DefaultTraceOperationsTest.java`, `DefaultTraceOperationsInSpanTest.java` | ISOLATE_V1_CHARACTERIZATION_TEST | v1 tests; migrate or isolate per plan §5 Slice 1B notes |
| `SpanRelation` usage | `platform-tracing-core/src/test/.../DefaultTraceOperationsBaselineTest.java` | ISOLATE_V1_CHARACTERIZATION_TEST | Slice 0A GREEN baseline evidence — must remain isolated, not deleted |
...
| Symbol | File | Classification | Slice 1B action |
|---|---|---|---|
| `recordException(...)` (abstract) | `TraceOperations.java` L253 | ALLOW_EXISTING_V1_UNTIL_CUTOVER | Method survives conceptually on `SpanHandle`/`TracingImplementation` in v3 (already added additively as `SpanHandle.recordException` in Slice 1A); old facade-level declaration removed at cutover |
| `recordException(...)` | `DefaultTraceOperations.java`, `MeteredPlatformTracing.java` (impl) | ALLOW_EXISTING_V1_UNTIL_CUTOVER / DELETE (Metered class) | Impl migrates to `TracingImplementation.recordException` boundary in Slice 2 |
| `recordException(...)` | `SpanScope.java` (separate v1 type, not `TraceOperations`) | FALSE_POSITIVE for `TraceOperations`-facade removal | `SpanScope.recordException` is a distinct v1 handle-level method; superseded by v3 `SpanHandle.recordException` (already exists). `SpanScope` itself is not in the required removal list and is out of Slice 1B scope unless the plan's Slice 1B text says otherwise (it does not explicitly list `SpanScope` for deletion) |
| `recordException(...)` | `platform-tracing-core/src/test/.../DefaultTraceOperationsBaselineTest.java` | ISOLATE_V1_CHARACTERIZATION_TEST | Slice 0A GREEN baseline evidence |
...
| `recordException(...)` | `platform-tracing-test/.../arch/OtelDirectIntegrationRules.java` | FALSE_POSITIVE | ArchUnit rule name (`NO_RAW_RECORD_EXCEPTION_OUTSIDE_RECORDER`) references a different, unrelated internal `ExceptionRecorder` concept, not the public `TraceOperations.recordException` facade method |
...
| `execute` (as public top-level method) | none found on `TraceOperations` or builders | — | Not present; already avoided |

## Inventory by module

| Module | Findings | Main action |
|---|---:|---|
| `platform-tracing-api` | 26 (declarations) + 9 (Facade/Abstract classes) = 35 | Delete v1 default methods and 9 Facade classes from `TraceOperations.java`/`span/builder/`; delete `SpanRelation.java` |
| `platform-tracing-core` | 5 production call sites + 9 test classes with v1 characterization usage | Migrate `DefaultTraceOperations`/`AbstractPlatformSpanBuilder` internals in Slice 2 (not 1B); isolate/migrate v1 tests |
...
|---|---|---|---|
| `DefaultTraceOperationsBaselineTest` | `platform-tracing-core` | Slice 0A GREEN baseline | Isolate as permanent v1 evidence, or migrate assertions to v3 API and retire once v3 equivalents exist per plan lifecycle rules |
...
| `DefaultTraceOperationsTest` | `platform-tracing-core` | Pre-existing v1 unit test | Migrate to v3 API assertions or isolate in a non-default source set at cutover |
| `DefaultTraceOperationsInSpanTest` | `platform-tracing-core` | Pre-existing v1 unit test | Same as above; covers `inSpan` semantics specifically |
...
- `docs/decisions/ADR-platform-tracing-micrometer-observation-boundary.md` — repository-facts table referencing `MeteredPlatformTracing`/`DefaultTraceOperations` as evidence
...
1. Treat `platform-tracing-api/src/main/java/.../TraceOperations.java` as the single source-of-truth diff: replace its ~26 declarations with `traceContext()` + `manual()` per plan §3.1, in one atomic commit together with deletion of the 9 `Facade*`/`AbstractFacadeTypedSpanBuilder` classes and `SpanRelation.java`.
...
4. Migrate or isolate the 12 v1 test classes listed above before merging; do not delete Slice 0A/0B evidence tests (`DefaultTraceOperationsBaselineTest`, `MeteredPlatformTracingKnownDefectTest`) — isolate into a non-default source set or leave running as-is if they still compile against a compatibility path, per plan lifecycle rules.
...
- No unexpected old-API usage was found in modules not covered by the plan (otel-extension, platform-tracing-test, and most of e2e/bench were confirmed false positives against the OTel SDK, not `TraceOperations`).
...
Follow-up disambiguation commands (used to separate `TraceOperations.startSpan` from OTel SDK `SpanBuilder.startSpan`):
...
rg -n "startSpan" -B 1 <file>   # inspect receiver type (Tracer.spanBuilder(...) vs TraceOperations instance)
```
