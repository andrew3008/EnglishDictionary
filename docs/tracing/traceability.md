...
| 3.1 | Outbound propagation платформенных заголовков (Фаза 12, agent-compatible) | OK | Secure-by-default DENY + trusted-host/topic gating: `OutboundPropagationPolicy`/`PlatformOutboundInjector`/`TrustedDestinationMatcher` (`platform-tracing-api`); HTTP — `PlatformOutboundHttpInterceptor` (webmvc) / `PlatformOutboundExchangeFilterFunction` (webflux); Kafka — `PlatformKafkaProducerInterceptor`. `propagate-force-trace=false` (sampled-flag несёт решение) |
| 3.2 | `X-Request-Id` = edge-stable correlation id (`request_id ≠ trace_id`), generate-if-absent + validate (CWE-113) | OK | `RequestIdSupport` (reject-and-regenerate); `TraceResponseHeader*Filter` (response `X-Request-Id` = correlation id, `X-Trace-Id` = trace-id); inbound-валидация унифицирована в `PlatformTraceControl.fromHeaders` (HTTP+Kafka). ADR-request-id-correlation-id |

## §2. Изоляция и отказоустойчивость

| # | Требование | Статус | Реализация |
|---|---|---|---|
| 1 | Неблокирующее поведение, ошибки только логируются | OK | `platform-tracing-otel-extension/.../processor/PlatformCompositeSpanProcessor.java` — обёртка-предохранитель span-pipeline (глотает исключения делегатов); `exporter/SafeSpanExporter.java` — изоляция исключений транспортного экспортёра (Фаза 10) |
...
| 7 | Соответствие OTel semantic conventions | OK | `platform-tracing-otel-extension/.../processor/ValidatingSpanProcessor.java` — runtime-валидация |
...
| GAP | Статус | Реализация |
|---|---|---|
| G2-01 high-level `inSpan()` API (Runnable / ThrowingSupplier / SpanRelation overloads) | OK | `PlatformTracing.inSpan(...)` — default-методы поверх `startSpan()` + `SpanScope.close()`; `ThrowingSupplier` (`platform-tracing-api/util`) для lambda-совместимости с checked-исключениями; покрыто `DefaultPlatformTracingInSpanTest` |
...
| P13-04 anti-double instrumentation (agent-first) | OK | модель B: `PlatformSpanContextKeys.PLATFORM_SPAN_CATEGORY` маркер + degradation в enrich (`AbstractPlatformSpanBuilder`); defense-in-depth ArchUnit `TracingArchRules.ESCAPE_HATCH_BUILDERS_REQUIRE_SUPPRESSION` + `@SuppressAgentInstrumentation`; первичный guard — agent-флаг `span-suppression-strategy=semconv` + e2e no-dup |
| P13-05 enrichment активного span'а | OK | `SpanEnricher.enrichCurrentSpan` (`GenericEnrichScope` — только platform-safe) / `enrichCurrentSpanIfPlatformCategory` (`EnrichScope` — allowlist); `SpanEnricherTest` |
...
| P15-02 named propagator SPI `platform-trace-control` | §3 (передача контекста) | OK | `propagation/PlatformTraceControlPropagatorProvider` (`ConfigurablePropagatorProvider`) + `META-INF/services/...ConfigurablePropagatorProvider`; `PlatformTraceControlPropagatorBuilder`; тест `PlatformTraceControlPropagatorProviderTest` |
